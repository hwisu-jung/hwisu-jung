...
삼육대학교 컴퓨터공학부 소프트웨어전공 4학년 · 온디바이스 LLM / Android
...
- 270M 모델이 없는 수치를 지어내서, 리포트 생성을 8단계 객관식으로 쪼개고 최종 문장은 앱에서 조립
- 응답 파싱에 실패하면 temperature를 바꿔 재시도, 계속 실패하면 그 영역만 빼고 리포트 완성
...
복지로(보건복지부) 공공 API에서 전국 17개 시·도 복지서비스를 모아 Firestore에 저장하고, 연령·성별·지역·소득분위에 맞는 정책을 추천하는 안드로이드 앱
